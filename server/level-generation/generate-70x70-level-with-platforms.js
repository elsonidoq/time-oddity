...
console.log('=== Time Oddity Cave Generation with Coin and Platform Placement Demo ===\n');
...
// Helper function to generate ASCII visualization without platforms
function generateMapWithoutPlatforms() {
...
// Print the level before platform placement
console.log('\n=== Level Before Floating Platform Placement ===');
console.log('Legend: . = floor, # = wall, P = player, G = goal, R = reachable area, C = reachable coin, c = unreachable coin');
console.log(generateMapWithoutPlatforms());

// 9. Floating Platform Placement
console.log('\nStep 9: Placing floating platforms...');
...
const candidatePlatforms = floatingPlatformPlacer.calculateOptimalPlatforms(connectedGrid, unreachableAreasForPlatforms, playerPos);
const validPlatforms = candidatePlatforms.filter(platform =>
  floatingPlatformPlacer.analyzer.validatePlatformPlacement(connectedGrid, platform)
);
const floatingPlatforms = floatingPlatformPlacer.refinePlacement(connectedGrid, validPlatforms);
console.log(`✅ Placed ${floatingPlatforms.length} floating platforms`);

// Helper function to generate ASCII visualization with platforms
function generateMapWithCoinsAndPlatforms() {
  const vizGrid = GridUtilities.copyGrid(connectedGrid);
  ...
  floatingPlatforms.forEach((platform, index) => {
    for (let x = platform.x; x < platform.x + platform.width; x++) {
      for (let y = platform.y; y < platform.y + (platform.height || 1); y++) {
        if (x >= 0 && x < width && y >= 0 && y < height) {
          GridUtilities.setSafe(vizGrid, x, y, 7); // 7=floating platform
        }
      }
    }
  });
...
// Print the final level with platforms
console.log('\n=== Final Level with Coins and Floating Platforms ===');
console.log('Legend: . = floor, # = wall, P = player, G = goal, R = reachable area, C = reachable coin, c = unreachable coin, F = floating platform');
console.log(generateMapWithCoinsAndPlatforms());
...
floatingPlatforms.forEach((platform, index) => {
  ...
  const visualImpact = floatingPlatformPlacer.assessVisualImpact(connectedGrid, platform);
...
console.log(`\nPlatform Summary:`);
console.log(`- Total platforms placed: ${floatingPlatforms.length}`);
console.log(`- Average platform size: ${floatingPlatforms.length > 0 ? (floatingPlatforms.reduce((sum, p) => sum + p.width * (p.height || 1), 0) / floatingPlatforms.length).toFixed(1) : 0} tiles`);
console.log(`- Average visual impact: ${floatingPlatforms.length > 0 ? (floatingPlatforms.reduce((sum, p) => sum + floatingPlatformPlacer.assessVisualImpact(connectedGrid, p), 0) / floatingPlatforms.length).toFixed(3) : 0}`);

// Print platform placement analysis
console.log('\n=== Platform Placement Analysis ===');
const groupedAreas = floatingPlatformPlacer.groupUnreachableAreas(unreachableAreasForPlatforms);

console.log(`- Unreachable areas for platforms: ${unreachableAreasForPlatforms.length}`);
